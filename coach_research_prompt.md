...
## ROLE
You are a world-class short-course triathlon coach and exercise physiologist. You have coached athletes from first-timers to draft-legal elites at the Sprint (750 m swim / 20 km bike / 5 km run) and Olympic / standard (1500 m / 40 km / 10 km) distances. Ground every claim in a NAMED methodology and say which one: Joe Friel (Triathlete's Training Bible, short-course), Brett Sutton (trisutto), Matt Dixon (Purplepatch / Fast-Track Triathlete), Stephen Seiler (polarized), Andrew Coggan (power), Jack Daniels (VDOT) and Pete Riegel for the run projection.

## WHAT THIS IS FOR (read carefully — it determines the format)
Your output is going to be ENCODED DIRECTLY into the rules of an AI triathlon coaching app. It is not advice for a human to read and interpret; it is a spec a programmer turns into code. So EVERY rule must be a constant, a lookup table, a formula, or a validator with an explicit numeric threshold. If a recommendation cannot be written as a number, a table row, or an if/then with thresholds, rewrite it until it can. No motivational language, no "it depends," no vague ranges without a default. When you must give a range, also give the single value the app should use by default.

The app does three things with your reference:
1. Generates a deterministic, periodized WEEKLY PLAN with hard numeric guardrails (it post-processes the AI's plan against your rules).
2. Projects a RACE FINISH TIME from the athlete's real training data.
3. Powers an LLM CHAT COACH that answers in your methodology's voice.

## SCOPE
Cover BOTH Sprint and Olympic. Mark EVERY divergence as [SPRINT vs OLY]. Critical framing that must run through every section: unlike Ironman, short course is raced AT or ABOVE threshold, so it is far more intensity-driven — the training, the zones, the projection fatigue factors, and the anti-patterns all differ from long course because of this.

## THE ATHLETE DATA THE APP ALREADY HAS (key every rule off these exact fields where relevant)
- goal_event (`sprint` or `olympic`), race_date, preferred_long_day, available_days (which weekdays the athlete can train), blocked days
- experience_level: `beginner` / `intermediate` / `advanced`; race_experience: `first_time` or has-raced-this-distance
- current_weekly_hours; equipment from {pool, open water, bike, running gear, gym}
- run_lthr (bpm), bike_lthr (bpm), swim CSS (mm:ss per 100 m); recent PBs (typically a 5K or 10K run time); age, sex, weight_kg; injuries
- a computed TRAINING-LOAD MODEL per athlete: CTL (42-day fitness), ATL (7-day fatigue), TSB (form = CTL−ATL), ACWR (acute:chronic workload ratio); plus per-session HR-drift %, and completed-vs-planned sessions for recent weeks.
Use these names. If a rule needs a datum we do NOT have (e.g. lactate, HRV), say so and give a fallback that uses only the data above.

## WHAT IS ALREADY BUILT — do NOT re-derive these; instead give the SHORT-COURSE-SPECIFIC numbers, and explicitly flag anything you would change
- Capped week-to-week volume ramp; guaranteed >=1 full rest day/week; <=2 sessions/day; no two same-energy-system HARD sessions on back-to-back days; tier-based deload cadence; week-to-week ADAPTIVE adjustment driven by HR drift + completion.
- A DETERMINISTIC PROJECTION engine already in code: run split = best run PB via Riegel (exponent ~1.06) times a TRI FATIGUE FACTOR; bike split = the athlete's aerobic speed times a RACE FACTOR; swim split = CSS + a buffer; plus T1/T2. (For Ironman the run fatigue factor is 1.12–1.20 and the bike race factor is ~0.97; for 70.3 ~1.05–1.12 and ~1.05.) Your job for section 8 is to give the equivalent SHORT-COURSE factor tables and confirm/correct the structure.
- A training LOAD MODEL fed to the coach; an LEA / low-energy-availability safety gate; youth-athlete volume caps.

## HOW TO FORMAT YOUR ANSWER (so it is directly codeable)
- Use the three ability tiers the app uses: `beginner` / `intermediate` / `advanced`. Give every tiered number as a 3-column lookup.
- Phases are: `base` / `build` / `peak` / `taper` / `race`. Give phase-dependent numbers as a table keyed by phase.
- Put EVERY numeric rule in a table or an explicit `key -> value` list. Prose is only for the one-line rationale ("why") strings the app shows users.
- End each section with a short "ENCODE AS:" line naming the constant/table/validator the app should create (e.g. "ENCODE AS: SHORT_TRI_FATIGUE[distance][tier] -> float").

Produce these sections:

1. PERIODIZATION MODEL — phase structure (base -> build -> peak -> short taper -> race). Give a TABLE mapping total-weeks-available -> number of weeks in each phase for 8, 12, 16, and 24-week plans [SPRINT vs OLY]. Give the rule to set phase boundaries from weeks-to-race ALONE (the app may not get a perfect plan length). Minimum viable build for a first-timer. Show, as percentages, how the short-course intensity mix shifts vs Ironman.

2. WEEKLY STRUCTURE & PROGRESSION (most important — all numbers). As tables keyed by tier and/or phase: max safe week-to-week volume increase (%/wk); deload cadence (every Nth week) + how much to cut (%); REST days/week; max sessions/day and the exact condition under which a double is allowed; a weekly-HOURS -> sessions-per-sport-and-their-durations map for Sprint (confirm ~5-9 h) and Olympic (confirm ~7-12 h); INTENSITY DISTRIBUTION as a "% of weekly time per zone, by phase" table (short course runs hotter than 80/20 — give the real split); and the SPORT-TIME split (swim/bike/run %) for SPRINT and OLY. Then list the guardrail validators with numbers (smooth load, always rest, never spike).

3. ZONES — three tables: run HR zones as % of run LTHR (Z1-Z5), bike HR zones as % of bike LTHR (Z1-Z5), swim zones from CSS. Then the exact RACE zone each leg is held at, for Sprint and for Olympic (as % LTHR and as a one-word effort label).

4. SESSION LIBRARY (per sport: swim, bike, run) — a table of every session type (recovery, easy/endurance, tempo, threshold, VO2/intervals, speed, long, technique/drills) with: target zone, typical duration or distance BY PHASE, and one-line purpose. Emphasize threshold + VO2 (what short course is built on). Then BRICKS in depth: frequency/wk, length, structure, and the race-pace brick simulation [SPRINT vs OLY], plus the bike-to-run "jelly legs" adaptation and the cue to give. TRANSITIONS (T1/T2): how to train them and the typical time cost in seconds [SPRINT vs OLY]. STRENGTH: sessions/wk by phase, type, and the minimum hours to keep it away from a key bike/run. OPEN-WATER vs pool skills (sighting, drafting, mass start) as a short checklist.

5. ADAPTATION TO THE ATHLETE — explicit rules (if/then with numbers) to scale to: available days/week, weekly-hours budget, tier, current fitness (run LTHR / bike LTHR or FTP / swim CSS / recent run PB), injuries, and equipment (pool vs open water, indoor trainer vs outdoor). Give the rule for PLACING the key swim, the threshold bike, the brick, and the long sessions across a 7-day week around the athlete's available_days while protecting recovery. State whether draft-legal (ITU) vs non-draft changes the bike prescription, and how.

6. TAPER — length and the volume/intensity changes for Sprint vs Olympic, as numbers (short races over-taper easily — quantify the % volume cut and what intensity to KEEP).

7. RACE-DAY EXECUTION (the app displays per-leg pacing) — a table: per leg (swim, bike, run), the target pace/effort as a % of LTHR/zone, for Sprint and for Olympic, plus ONE one-line "why" string per leg that a smart athlete would respect (these strings are shown verbatim — write them well). Include how hard to push the bike while still being able to run fast off it. Transition strategy. Fueling/hydration per leg as carbs/hr + fluid + sodium numbers (minimal for sprint, modest for olympic).

8. RACE-TIME PROJECTION METHODOLOGY (most code-critical — be exhaustive and exact). Give:
   - The RUN-split formula: open-run prediction from the best run PB via Riegel (state the exponent), THEN the SHORT-COURSE TRI FATIGUE multiplier applied to get the off-the-bike run. Give that multiplier as a table [Sprint, Olympic] x [beginner, intermediate, advanced]. (Reminder: it is much smaller than Ironman's 1.12-1.20 — likely close to 1.0 for sprint.)
   - The BIKE-split method from FTP/NP OR from average aerobic speed, times a short-course RACE FACTOR (table [Sprint, Olympic], and the FTP%/IF the bike is ridden at). Give the default bike speed to assume when the athlete has no ride data, by tier.
   - The SWIM-split method: CSS + buffer (give the buffer in s/100m for Sprint and Olympic), and the default to assume when CSS is missing.
   - T1/T2 time estimates in seconds [SPRINT vs OLY].
   - MINIMUM data for a credible projection; what raises/lowers confidence; and explicit LOW / MEDIUM / HIGH confidence criteria as data thresholds (e.g. "HIGH = has CSS AND >=X weeks data AND a run PB within Y%").
   - How to express the result as a +/- % RANGE per confidence level.
   - WORKED EXAMPLE, step by step, for BOTH a full Sprint and a full Olympic: athlete with 5K PB 21:12, bike ~26 km/h aerobic (FTP ~210 W), swim CSS 1:56/100m, ~10 weeks consistent, intermediate tier. Show swim split, T1, bike split (show the speed/time math), T2, run split (open 5K/10K prediction then fatigue factor), total, the confidence label, and the range. I will use these numbers to UNIT-TEST the encode, so they must be internally consistent.

9. ANTI-PATTERNS / SAFETY — DO-NOT rules with numbers. Short-course-specific: too much slow aerobic volume and not enough threshold/VO2 (the OPPOSITE of the IM mistake); or all-intensity-no-base; neglecting the swim because it is "only 10-15% of the race"; never practicing bricks/transitions; over-tapering a short race; any load spike / missing recovery week / unrealistic ramp. Each as a validator with a threshold and the corrective action.

10. FEASIBILITY / SANITY CHECK — given a goal time + weeks-to-race + current fitness, the rule to judge if it is realistic, and the MINIMUM training (weekly hours, number of weeks, key sessions/wk, longest brick) a given Sprint or Olympic goal time requires. Pair any "unrealistic" verdict with the specific lever (more weeks, more volume, more intensity, or a softer goal). Give this as a small table of goal-time bands -> minimum requirements.

FORMAT: headers + bullets; a TABLE for every set of numbers; "ENCODE AS:" line per section; mark every [SPRINT vs OLY]; default value given for every range; dense, zero filler.

---

# Coach pair 4 round 2 follow-up (paste into the SAME sprint/olympic conversation)

That reference is excellent and I have already encoded the periodization, the short-course intensity mix, the zones, the brick rules, the race-pace targets, and the deterministic projection (run PB + short-course fatigue factor, bike speed times a race factor, swim from CSS, T1/T2) into the app. Now I want to go deeper and make sure we are not missing anything, because for this app the quality of the coach is everything. Same rules as before: every recommendation must be CODEABLE (constant / table / formula / validator-with-threshold), keyed off the athlete data fields I listed, with an "ENCODE AS:" line per section. Keep covering BOTH Sprint and Olympic, mark every [SPRINT vs OLY], stay dense and numeric, ground it in Friel / Sutton / Dixon / Seiler / Coggan, and give a default value for every range.

Two asks first:

A) REVIEW AND EXTEND your own reference. Re-read what you wrote and add anything you left out, glossed over, or could make more precise or codeable. Same style.

B) Give me YOUR expert opinion on what a truly great AI short-course triathlon coach should have that I did not think to ask for. Do not hold back. If there is a whole topic we are missing (open-water race craft, drafting tactics, transition speed, fast-twitch / run-speed development for the 5K off the bike, female-athlete physiology and cycle-based adjustments, masters athletes, youth / draft-legal development), add it as a codeable section.

Then go deep on these specific areas. For each give hard numbers, formulas, tables, an "ENCODE AS:" line, and at least one worked example:

1. ADAPTIVE ADJUSTMENT (most important). Inputs the app can compute per completed week: HR drift % per session, actual pace/power/speed vs target, sessions completed vs missed, the CTL/ATL/TSB/ACWR load model, RPE + felt-tags (easy/hard/sore/sick) when logged, and the 2-3 week trend. Give explicit if/then rules with thresholds for how to adjust NEXT week (cut volume X%, hold, repeat, early deload, drop intensity). Include the short-course-specific case: the athlete is HITTING volume but the THRESHOLD/quality sessions are fading or drifting — the signal to back off INTENSITY, not volume. List overtraining / under-recovery red flags (with the ACWR / TSB / drift thresholds that trigger them) and the response to each.

2. IN-SESSION STRUCTURE. For each session type per sport (and the brick): a warmup / main set / cooldown template with durations and targets, and how to SCALE the reps/volume by PHASE (show a threshold bike and a VO2max run growing base -> build -> peak). These become the workout `description` strings, so make them concrete and prescriptive.

3. KEY-SESSION PLACEMENT across the week. Exact spacing rules (what counts as the SAME energy system across swim/bike/run, and the minimum gap in days), plus ready-to-use 7-day weekly templates for 5-, 6-, and 7-day availability, for SPRINT and OLY, in base / build / peak, showing where the brick, the key swim, the threshold bike, rest, and the deload week sit. Give these as day-by-day tables.

4. PROJECTION DEPTH. More worked examples (same numeric rigor, must be unit-testable): a swim-limited athlete, a strong cyclist / weak runner, a draft-legal vs non-draft Olympic athlete, and a Sprint-to-Olympic step-up. How to UPDATE the projection as fitness changes mid-block, and how a recent BRICK-run pace (run pace held immediately off the bike) can sharpen — or override — the fatigue-factor run-split estimate.

5. RECOVERY & READINESS. How to read fatigue from resting HR / HRV / sleep / mood when available, and from training data (pace/power at fixed HR, RPE creep, drift, TSB) when not; concrete thresholds for inserting an unplanned rest day or cutting a week short.

6. ENVIRONMENT & LOGISTICS. Heat / cold-water / wetsuit-legal water temperature / altitude adjustments (numbers), hilly-bike and hot-run pacing, and a race-week + race-morning checklist specific to short course (transition setup, and the warm-up that genuinely matters for a 5K-off-the-bike).
...
8. PROACTIVE COACHING. The specific situations a good short-course coach should NOTICE from the data and message the athlete about unprompted (missed bricks, easy sessions creeping into the grey zone, threshold sessions stalling, neglected swim, ACWR spike, taper anxiety, race week, new PB). Give each as: trigger condition (with the data threshold) -> message intent.

Format: dense, hard numbers, tables, formulas, worked examples, "ENCODE AS:" lines, [SPRINT vs OLY] marked, no filler. Give me everything; if in doubt, include it.
