...
>
> 🔜 **STILL TODO (the one big remaining): Share-page STRUCTURE rework** (task #30) — drop the week-picker in `openShareModal`; `renderPublicShare` shows a plan-structure explainer + the LAST COMPLETED week with key stats per workout + the try-the-app CTA.
>
...
>
> 🔜 **STILL TODO (the one remaining from Noah's list): Share-page STRUCTURE rework.** Drop the week-picker in the share modal (`openShareModal`) — a 50-week list is unusable. Instead the public page (`renderPublicShare`) should always show: a clear plan-structure explainer (phases / weekly pattern) + the athlete's LAST COMPLETED week with key stats per workout (most important stats, not all). Keep the try-the-app CTA. Needs changes to both the modal (remove week selection, maybe store nothing or a flag) and the page (compute last completed week from `workouts` + plan structure from `training_plans`).
>
...
>
> 🔜 **STILL TODO from this batch (Noah's list, deferred — bigger):** (A) **Strava-style NOTIFICATIONS** — bell icon by the avatar with a red dot, slide-in panel; notifications like "logged a new workout" + "we're not sure if this was planned/extra, can you clarify?" Needs a `notifications` table (or reuse) + UI + generation hooks on workout sync/classification. (B) **Share-page structure rework** — remove the week-picker in the share modal (a 50-week list is unusable); instead always show a clear plan-structure explainer + the LAST completed week with key stats per workout; keep the try-the-app CTA.
>
