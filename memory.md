...
>
> ▶ **CURRENT STATE (start here, 2026-05-23):** Frontend **v79** live (Vercel auto-deploys on push to `noahb-developer/stryxs`). Local clone == origin (clean, pushed from local this session). No edge deploy this round (frontend only). `migrate_v7.sql` APPLIED (workouts UPDATE policy — confirmed via pg_policies). This session = round 11 (a→c), a punch-list of fixes before a big task. **CONFIRMED WORKING by Noah:** Log-manually Date/Duration box (don't touch), classify-from-notification/analyze now persists (was the missing RLS UPDATE policy), opened past workouts clear from the Analyze page on re-entry. **AWAITING NOAH'S iPhone CHECK on v79 (he must HARD-REFRESH — PWA cache is the usual "didn't work" cause):** (a) do race projections now climb off "0/5" as he confirms/logs ≥5 training workouts in the last 6 weeks (v79 killed a stale week-long low-data cache + relaxed the window)? (b) does the Plan page now fill in ACTUAL vs PLANNED when he opens a completed session (v79 new feature, render-time match by sport+date)? **FIRST THINGS NEXT SESSION:** (1) `git fetch && git status` and reconcile before editing (local clone has been stale before — see lesson below). (2) Confirm (a)+(b) above with Noah, then ask about his **"very big" next task** — he's mentioned it for ~3 messages and wants to start it once these are solid. Possible future items: real completed-sensor stats on the public share (needs `shared_plans.snapshot` jsonb + migration), Coach reading non-image files, persisting sent chat photos, i18n/marketing threads further down.
>
