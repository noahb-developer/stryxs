...
> • **(1) Spinner wobble** — `.spinner` now GPU-promoted (`will-change:transform` + centered `transform-origin` + backface-hidden) so the rotation composites on a stable pixel grid instead of sub-pixel jittering next to flex-centered text. Commit d3cfc6d.
...
> • **(8) Settings deep-link scroll** — buttons that open Settings to a section (e.g. auto-sync → "Manage connections") landed the header UNDER the fixed ~56px nav (looked "too low"). `toggleSettingsSection` now uses computed `scrollTo(absoluteTop - 72)` (same pattern as the spotlight tour) so the section starts cleanly below the nav. Commit d3cfc6d.
