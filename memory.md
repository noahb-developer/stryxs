...
>
> ▶ **CURRENT STATE (start here, 2026-05-23):** Frontend **v87** live (Vercel auto-deploys on push to `noahb-developer/stryxs`). **Coach edge DEPLOYED many times this session** via `cd /c/Users/noahb/enduraiq-functions && supabase functions deploy coach --no-verify-jwt` (CLI is authenticated — **Claude Code CAN deploy edge functions itself**, no need to ask Noah). Local stryxs clone == origin (clean). `migrate_v7.sql` applied (workouts UPDATE RLS policy). **This was a HUGE session: rounds 11→14 = the full COACH/PLAN-QUALITY OVERHAUL** (the launch blocker). Research workflow: prompts in `coach_research_prompt.md`, methodology saved in `coach_methodology_reference*.md` (tri, marathon ×2, 5k10k+general). **WHAT'S DONE & DEPLOYED:** plan-gen guardrails (R1 ≥1 rest, R3 ≤2/day, R6 no back-to-back same-sport hard, capped volume ramp, tier deload cadence); adaptive week-to-week adjustment; youth-athlete safety cap; LEA/iron safety gate; deterministic projections — TRI (computeTriProjection: real bike km/h + per-leg why + confidence/range) and RUNNING (computeRunProjection: VDOT+Riegel+marathon mileage-fade); training LOAD MODEL (CTL/ATL/TSB/ACWR dashboard card, fed to Coach); proactive nudges (bell); concrete WU/main/CD session detail; persona-specific plan rules for IM/70.3, marathon/half, short_run, AND general (Attia 4-pillar); general-fitness progress panel (est VO2max + aerobic-efficiency trend, v87). **COACHES STATUS:** ✅ Ironman+70.3 (full, round 1+2) · ✅ Marathon+Half (full, round 1+2) · ✅ 5K/10K+General (plan rules + general panel; round-2 optional) · ⏳ short_tri (sprint/olympic) = pair 4, projection already works via computeTriProjection, just needs persona-prompt polish + optional research. **FIRST THINGS NEXT SESSION:** (1) `git fetch && git status`. (2) Ask Noah to TEST the coaches end-to-end on his iPhone (hard-refresh): regenerate plans for a couple goal types, check projections/bike-speed/load-card/general-panel, and whether plans feel good. (3) Then either finish pair 4 (sprint/olympic), do round-2 for pair 3, OR move to whatever Noah raises. Possible older items: completed-sensor stats on public share (needs `shared_plans.snapshot` jsonb + migration), Coach reading non-image files, persisting sent chat photos, i18n/marketing.
>
