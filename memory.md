...
>
> ▶ **CURRENT STATE (start here, 2026-05-23):** Frontend **v80** live (Vercel auto-deploys on push to `noahb-developer/stryxs`). Local clone == origin (clean, pushed from local this session). No edge deploy yet this stretch (frontend only) — but the COACH OVERHAUL above WILL need a coach-edge deploy. `migrate_v7.sql` APPLIED (workouts UPDATE policy). **CONFIRMED WORKING by Noah (round 11):** Log-manually Date/Duration box (don't touch), classify-from-notification/analyze persists (RLS fix), opened past workouts clear from Analyze on re-entry, race projections climb off 0/5 (v79), Plan page actual-vs-planned fill-in (v79). v80 fixed the confidence mismatch + added the "more accurate with data" note. **FIRST THINGS NEXT SESSION:** (1) `git fetch && git status` and reconcile before editing. (2) See the 🚀 NEXT BIG PUSH block at top: if Noah has pasted research output → start encoding it into the coach edge (Ironman+70.3). If not yet → do the CONTAINED TODO items (per-leg pace/why on projections card, the 2 small bugs). Possible later items: completed-sensor stats on public share (needs `shared_plans.snapshot` jsonb + migration), Coach reading non-image files, persisting sent chat photos, i18n/marketing.
>
