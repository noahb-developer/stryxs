...
>
> ⚠️ **PENDING DEPLOY (2026-05-23): the `coach` edge function has un-deployed changes (round 12a, plan-gen guardrails).** Noah must run, from `C:\Users\noahb\enduraiq-functions`: `supabase functions deploy coach --no-verify-jwt`. The edits are local at `supabase\functions\coach\index.ts` (NOT a git repo — deploy via supabase CLI). Until deployed, plans still spike. After deploy: regenerate a plan (Plan tab → Plan Management → Generate new plan) and check the easy→hard jumps / 0-rest-days / 4x-a-day are GONE. TypeScript syntax verified (tsc --noResolve, no TS1xxx errors). See round 12a below.
>
