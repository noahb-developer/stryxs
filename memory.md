...
>
> ▶ **CURRENT STATE (start here, 2026-05-24):** Frontend **v87** live (Vercel auto-deploys on push to `noahb-developer/stryxs`). **Coach edge DEPLOYED** via `cd /c/Users/noahb/enduraiq-functions && supabase functions deploy coach --no-verify-jwt` (CLI is authenticated — **Claude Code CAN deploy edge functions itself**, no need to ask Noah). Local stryxs clone == origin (clean). `migrate_v7.sql` applied (workouts UPDATE RLS policy). **The COACH/PLAN-QUALITY OVERHAUL (the launch blocker) is rounds 11→15: pairs 1-3 fully research-backed + live; pair 4 (short_tri) is the LAST one and is awaiting Noah's research paste (interim rules deployed).** Research workflow: prompts in `coach_research_prompt.md`, methodology saved in `coach_methodology_reference*.md` (tri, marathon ×2, 5k10k+general; short_tri is INTERIM/placeholder until Noah's research). **WHAT'S DONE & DEPLOYED:** plan-gen guardrails (R1 ≥1 rest, R3 ≤2/day, R6 no back-to-back same-sport hard, capped volume ramp, tier deload cadence); adaptive week-to-week adjustment; youth-athlete safety cap; LEA/iron safety gate; deterministic projections — TRI (computeTriProjection: real bike km/h + per-leg why + confidence/range, all 4 distances) and RUNNING (computeRunProjection: VDOT+Riegel+marathon mileage-fade); training LOAD MODEL (CTL/ATL/TSB/ACWR dashboard card, fed to Coach); proactive nudges (bell); concrete WU/main/CD session detail; persona-specific plan rules for ALL personas — IM/70.3, marathon/half, short_run (now 5K-vs-10K aware), general (Attia 4-pillar), AND short_tri (short-course Friel); general-fitness progress panel (est VO2max + aerobic-efficiency trend, v87). **COACHES STATUS:** ✅ Ironman+70.3 (research round 1+2) · ✅ Marathon+Half (research round 1+2) · ✅ 5K/10K+General (research round 1, plan rules incl. 5K-vs-10K mix + general panel) · 🟡 short_tri sprint/olympic — projection live + INTERIM Claude-synthesized plan rules (round 15), but **the deep research is still TO BE RUN by Noah** (see workflow note below). **RESEARCH WORKFLOW (important):** Noah runs the research prompt (in `coach_research_prompt.md`) through a strong model and pastes the rich reference back; Claude Code encodes it. The **Pair-4 (Sprint+Olympic) round-1 + round-2 prompts are now in `coach_research_prompt.md`, rewritten to be ENCODING-OPTIMIZED** (they brief the research model on the app's exact data fields, what's already built, demand codeable lookup-table output + "ENCODE AS:" lines + a unit-testable worked projection example). Waiting for Noah to run round-1 and paste back. **FIRST THINGS NEXT SESSION:** (1) `git fetch && git status`. (2) **Did Noah paste the Pair-4 research output?** If yes → REPLACE the interim `coach_methodology_reference_short_tri.md` with it, re-encode the deeper detail (real fatigue/race-factor tables, open-water, draft-legal, brick periodization), VERIFY the worked projection example matches the encode, then deploy. Then Noah runs round-2 in the same chat → paste → layer in. ENCODE ONLY what Noah pastes — do NOT write more coach rules from your own knowledge while waiting (that caused a tangle this session), and do NOT re-touch the pairs 1-3 prompts (Noah said leave them; only re-touch if re-running that pair). See [[coach-research-workflow]] feedback memory. (3) Optionally ask how the interim short_tri/5K/10K rules felt on iPhone (regenerate plans, check intensity/bricks/projection). (4) Remaining research (later, optional): Pair-4 round-2, round-2 for pair 3 (general 4-week A/B/C/D rotation). Non-research queue: `workout_feedback` RPE UI (§7), completed-sensor stats on public share (needs `shared_plans.snapshot` jsonb + migration), Coach reading non-image files, persisting sent chat photos, i18n/marketing.
>
