...
>
> ⚠️ **PENDING DB ACTION (2026-05-23): Noah must run `migrate_v7.sql` in Supabase Studio → SQL Editor.** ROOT CAUSE of "classify keeps asking / Could not update": the `workouts` table has SELECT + INSERT RLS policies but **NO UPDATE policy**, so `classifyWorkout`'s write matched 0 rows and silently failed (Supabase returns error:null on a 0-row update). Reads work, writes don't → classification never persisted → notifications kept asking AND synced workouts stayed `unclassified` so Race Projections sat at 0/5 (issues #1 and #3 are the SAME bug). The frontend (v78) is already correct (verifies with `.update().select()`, reports failure) — it just can't write until the policy exists. `migrate_v7.sql` (in repo root) adds `workouts_update_own` (FOR UPDATE, auth.uid()=user_id), idempotent. Until Noah runs it, classify will keep failing. CHECK NEXT SESSION whether he ran it (ask, or have him retry classifying a workout).
>
> ▶ **CURRENT STATE (start here, 2026-05-23):** Frontend **v78** live (Vercel auto-deploys on push to `noahb-developer/stryxs`). Local clone == origin (clean, pushed from local this session). No edge deploy this round (frontend only). One **pending DB migration: `migrate_v7.sql`** (see ⚠️ above). **FIRST THINGS NEXT SESSION:** (1) `git fetch && git status` and reconcile before editing (local clone has been stale before — see lesson below). (2) Ask Noah (after he HARD-REFRESHES his iPhone — PWA cache is the usual reason a fix "didn't work"): does classifying from a notification/analyze page now stick (no longer keeps asking)? Does the Log-manually Date box match Duration? Do race projections move off "0/5" once he confirms/logs ≥5 training workouts? Does an opened past workout now disappear from the Analyze page when he leaves & returns? He has a **"very big" next task** queued to start once these are confirmed working — ask what it is. Possible future items: real completed-sensor stats on the public share (needs `shared_plans.snapshot` jsonb + migration), Coach reading non-image files, persisting sent chat photos, i18n/marketing threads further down.
>
